...
const CardMovie = () => {
  ...
  return (
    <div style={{ display: 'flex', margin:'2em' , padding: '2em'}}>{movie.result !== undefined && movie.result.map((item) => {
return(
  <CardColumns  className="my-carousel" style={{ display: 'flex', padding: '2em', margin:'2em'}}>
      <Link to={`/DetailMovie/${item._id}`} key={item._id}>
      <Card className="my-carousel" style={{  width: '10rem'}}>
        <Card.Img  style={{ height: '10rem'}} variant="top" src={item.image_url} />
